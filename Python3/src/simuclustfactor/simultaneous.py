...
_doc_init_args = '''
	Initialization Args:
	--------------------
	n_max_iter (int, optional): number of iterations. Defaults to 10.
	n_loops (int, optional): number of loops to gurantee global results. Defaults to 5.
	tol (float, optional): tolerance/acceptable error. Defaults to 1e-12.
	random_state (int, optional): seed for random generations. Defaults to None.
...
_doc_fit_args = '''
	...
	reduced_tensor_shape (1D array): (G,Q,R) dimensions of centroid tensor.'''

# accessible result/outputs
_doc_init_attrs = '''
	Attributes:
	-----------
	Y_g_qr (ndarray): (G,QR) matricized version of three-way centroid arrays.
...
# === The T3CLUS MODEL
@_doc_formatter(_doc_init_args, _doc_fit_args, _doc_init_attrs, _doc_refs)
class T3Clus(_BaseClass):
	"""
	Simultaneous version of TWCFTA - Clustering-Factorial Decomposition.
	{0}
	{1}
	{2}
	{3}
	...
	def fit(self, X_i_jk, full_tensor_shape, reduced_tensor_shape, U_i_g=None, C_k_r=None, twcfta_init='random', twfcta_init='random'):
		...
		{0}
		U_i_g (ndarray): IxG membership function matrix.
		C_k_r (ndarray): KxR occasions component matrix.
		{1}
		"""

		# alpha = 1
		# return self.model.fit(X_i_jk=X_i_jk, full_tensor_shape=full_tensor_shape, reduced_tensor_shape=reduced_tensor_shape, alpha=alpha, twcfta_init=twcfta_init, twfcta_init=twfcta_init)
...
class CT3Clus(_BaseClass):
	"""
	Combination of T3Clus and TFKMeans. equation (38)
	Minimize within cluster deviance.
	{0}
	{1}
	{2}
	{3}
	...
	def fit(self, X_i_jk, full_tensor_shape, reduced_tensor_shape, alpha=0.5, twcfta_init='random', twfcta_init='random'):
		'''
		Combined T3Clus and 3FKMeans (CT3Clus)
		{0}
		alpha (float): 0<=a<=1. a=1 for T3Clus, a=0 for TFKMeans. a=0.5 improve recoverability.
		{1}
		'''
		# initializing basic config
		rng = np.random.default_rng(self.random_state)  # random number generator
