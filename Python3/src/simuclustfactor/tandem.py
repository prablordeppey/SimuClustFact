...
_doc_fit_args = '''
	...
	full_tensor_shape (1D array): (I,J,K) dimensions of the original tensor.
	reduced_tensor_shape (1D array): (G,Q,R) dimensions of centroids tensor.'''

# accessible result/outputs
_doc_init_attrs = '''
	Attributes:
	-----------
	Y_g_qr (ndarray): (G,QR) matricized version of three-way core tensor.
	U_i_g (ndarray): (I,G) stochastic membership function matrix.
	B_j_q (ndarray): (J,Q) component weight matrix for variables.
	C_k_r (ndarray): (K,R) component weight matrix for occasions.
	E_i_jk (ndarray): (I,JK) matricized version of three-way residual terms array E_i_j_k.
	metrics (dict): associative array as keys; BSS, TSS, WSS, PseudoF
	converge_status (bool): whether the algorithm converged or not'''

# output specific to the twcfta model
_doc_attrs_twcfta = '''
	X_g_jk (ndarray): (G,JK) matricized three-way centroids arrays.
	E_g_jk (ndarray): (G,JK) matrix of three-way residual arrays. Matricized E_g_j_k with frontal slabs next to each other.'''

# output specific to the twfcta model
_doc_attrs_twfcta = '''
	Attributes:
	-----------
	Y_i_qr (ndarray): (I,QR) matrix of component scores. Thus, matricized version of Y_i_q_r
	E_i_qr (ndarray): (I,QR) matrix of three-way residual arrays. Matricized E_i_q_r with frontal slabs next to each other.'''
...
# === The TWCFTA MODEL
@_doc_formatter(_doc_init_args, _doc_fit_args, _doc_init_attrs, _doc_attrs_twcfta, _doc_refs)
class TWCFTA(_BaseClass):
	"""
	Three-way Clustering-Factorial Tandem model (TWCFTA).
	1). Perform KMeans on X_i_jk to obtain membership matrix U_i_g and centroids X_g_jk in full space.
	2). Obtain Y_g_qr and C_k_r & B_j_q factor matrices that maximizes the reconstruction of U_i_g.X_g_jk via Tucker2.
	{0}
	{1}
	{2}
	{3}
	...
	@_doc_formatter(_doc_fit_args, _doc_init_attrs, _doc_attrs_twcfta)
	def fit(self, X_i_jk, full_tensor_shape, reduced_tensor_shape):
		"""
		Three-way Clustering-Factorial Tandem Analysis (TWCFTA)

		1). Clustering on objects (via KMeans applied to X_i_jk)
		2). Factorial decomposition on centroids (via Tucker2 applied to the centroids matrix X_g_jk_bar)
		{0}
		{1}
		{2}
		"""
		...
		# total time taken
		self.TotalTimeElapsed = sum(best_elapsed_times)
...
# === The TWFCTA MODEL
@_doc_formatter(_doc_init_args, _doc_fit_args, _doc_init_attrs, _doc_attrs_twfcta, _doc_refs)
class TWFCTA(_BaseClass):
	...
	@_doc_formatter(_doc_fit_args, _doc_init_attrs, _doc_attrs_twfcta)
	def fit(self, X_i_jk, full_tensor_shape, reduced_tensor_shape):
		"""
		Three-way Factorial - Clustering Tandem Analysis (3WFCTA)
		
		1). Factorial Decomposition (via Tucker2 applied to X_i_jk) to obtain C_k_r and B_j_q component factors.
		2). Clustering (via KMeans applied to Y_i_qr) to obtain U_i_g and Y_g_qr.
		{0}
		{1}
		{2}
		"""
